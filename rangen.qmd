...
relation as
$$
X_{n + 1} = \left\lfloor \frac{X_n}{b^\frac{e}{2}} \right\rfloor \mod b^e
$$
...
go about that. It is even ok if you do not commit to permanent memory the precise
conditions of this theorem. But let's take a closer look at what they imply in
this particular case. The prime factors of $m$ are 2 and 5, so we want $a - 1$
to be divisible by both. Actually, since $m$ is divisible by 4, we want $a -1$
to be divisible by both 4 and 5, which is to say
$$
a \mod 20 = 1
$$
...
choice. (Try it out if you are not convinced.) The other thing is that we want
$c$ coprime with 100, which means that it should be odd and not a multiple of 5.
...
On a second thought: are we *really* happy with this?
