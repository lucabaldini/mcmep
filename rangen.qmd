...
to how a typical pseudo-random number generator works. The mathematical equivalent
of the concept of "wrapping around at every turn" is the *modulo* operation in
ordinary integer arithmetic and it is interesting to note that, as we shall see
in a moment, the modulo is a fundamental ingredient in all the generators that are
relevant for everyday use.
We shall briefly come back to modular arithmetic in section @sec-modular-arithmetic
but, before we do that, there's a few thing we should sort out.
...
(amusing, I promise) before you consider opting away from a TRNG.
